      ...
      <div className="bg-surface border border-border rounded-2xl w-full max-w-[660px] max-h-[92vh] flex flex-col shadow-[0_20px_80px_rgba(0,0,0,0.6)]"
          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            {discountPct !== null && (
              <div className="col-span-2 flex items-center gap-4 px-5 py-4 bg-green/5 border border-green/20 rounded-xl">
                ...
                <span className="text-xs text-muted line-through">${Number(form.originalPrice).toLocaleString()}</span>
                <span className="text-xs text-muted">→</span>
                <span className="text-sm font-bold text-text">${Number(form.price).toLocaleString()}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 px-7 py-5 border-t border-border shrink-0 bg-surface2/50">
          ...
          <button onClick={onSave} disabled={saving}
            className="px-6 py-2.5 bg-accent text-white rounded-xl font-display font-bold text-sm hover:bg-[var(--color-accent-hl)] transition-colors disabled:opacity-40 min-w-[150px] inline-flex items-center justify-center gap-2 shadow-[0_2px_14px_rgba(108,95,255,0.4)]">
...
export default function AdminProducts() {
  ...
  }, [search, category, page, sortKey, sortDir, stockFilter]);
      ...
      case "_actions": return (
        <div className="flex gap-2">
          <button onClick={() => openEdit(row)}
            className="px-4 py-2 text-xs font-bold font-display bg-surface border border-border rounded-xl hover:border-accent hover:text-accent hover:bg-accent/5 transition-all shadow-sm">
            Edit
          </button>
          <button onClick={() => setConfirmDel(row)}
            className="px-4 py-2 text-xs font-bold font-display bg-transparent border border-red/25 text-red rounded-xl hover:bg-red/10 hover:border-red/40 transition-all">
            Delete
          </button>
        </div>
      );
    ...
    <div className="p-6">
      ...
      {!loading && (
        <MetricRow items={[
          ...
          ["Avg Price",    `$${avgPrice.toLocaleString()}`, "per item"],
          ["Out of Stock", outOfStock,                 "items", outOfStock > 0 ? "danger" : null],
          ["Low Stock",    lowStock,                   "< 5 units", lowStock > 0 ? "warn" : null],
        ]} />
      )}
      ...
      {modal && (
        <ProductModal modal={modal} form={form} onChange={setForm} saving={saving} onSave={handleSave} onClose={() => setModal(null)} />
      )}
