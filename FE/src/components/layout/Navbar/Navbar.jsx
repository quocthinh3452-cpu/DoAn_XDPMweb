...
import MiniCartPreview from './MiniCartPreview.jsx';
...
export default function Navbar({
  ...
  // Đã sửa lỗi: Chỉ dùng cartItems từ CartContext
        ...
        <div className="container-page h-full flex items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {wishCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 bg-rose-500 text-white text-xs font-bold min-w-5 h-5 rounded-full flex items-center justify-center px-1 shadow-sm" aria-hidden>
                  {wishCount > 99 ? "99+" : wishCount}
                </span>
              )}
            </Link>

            {/* Khối Icon Giỏ Hàng (Đã đồng bộ giao diện) */}
            <div className="relative group flex items-center h-full">
              <Link 
                to="/cart" 
                aria-label={`Giỏ hàng${itemCount > 0 ? ` (${itemCount})` : ""}`}
                className={`relative w-10 h-10 flex items-center justify-center bg-surface border border-border rounded-lg transition-all duration-200 ${pathname === "/cart" ? "border-accent text-accent" : "text-muted hover:border-accent hover:text-accent"}`}
              >
                ...
                {/* Chấm đỏ báo số lượng sản phẩm */}
                {itemCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-blue-600 text-white text-xs font-bold min-w-5 h-5 rounded-full flex items-center justify-center px-1 shadow-sm" aria-hidden>
                    {itemCount > 99 ? "99+" : itemCount}
                  </span>
                )}
              </Link>

              {/* Bảng MiniCart ẩn đi, hiện ra khi hover */}
              <div className="hidden group-hover:block absolute right-0 top-[100%] pt-4 z-50">
                <MiniCartPreview />
              </div>
            </div>
